...
def test_prop_with_nested_property():
    nd = NgsiDict()
    p = nd.build_property(
        22, unitcode="GP", userdata={"accuracy": nd.build_property(0.95)}
    )
    assert p == {
        "type": "Property",
        "value": 22,
...
def test_geoprop_point():
    ...
    p = NgsiDict().build_geoproperty(bx)
    assert p == {
        "type": "GeoProperty",
...
def test_geoprop_point_as_tuple():
    p = NgsiDict().build_geoproperty((44.84044, -0.5805))
    assert p == {
        "type": "GeoProperty",
        "value": {"coordinates": [-0.5805, 44.84044], "type": "Point"},
    }
...
def test_temporal_prop_datetime():
    p = NgsiDict().build_temporal_property(datetime(2021, 8, 31, 12))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_datetime_str():
    p = NgsiDict().build_temporal_property("2021-08-31T12:00:00Z")
    assert p == {
        "type": "Property",
        "value": {"@type": "DateTime", "@value": "2021-08-31T12:00:00Z"},
    }
...
def test_temporal_prop_date():
    p = NgsiDict().build_temporal_property(date(2021, 8, 31))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_date_str():
    p = NgsiDict().build_temporal_property("2021-08-31")
    assert p == {
        "type": "Property",
        "value": {"@type": "Date", "@value": "2021-08-31"},
    }
...
def test_temporal_prop_time():
    p = NgsiDict().build_temporal_property(time(12, 0, 0))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_time_str():
    p = NgsiDict().build_temporal_property("12:00:00Z")
    assert p == {
        "type": "Property",
        "value": {"@type": "Time", "@value": "12:00:00Z"},
    }
...
def test_temporal_prop_str_bad_format():
    with pytest.raises(ValueError):
        p = NgsiDict().build_temporal_property("25:00:00Z")
