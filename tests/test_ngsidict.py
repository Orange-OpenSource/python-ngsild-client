...
from ngsildclient.model.ngsidict import Attr
from ngsildclient.model.constants import AttrValue


def test_prop():
    p = Attr()._build_property(AttrValue(22))
    assert p == {"type": "Property", "value": 22}


def test_prop_string():
    p = Attr()._build_property(AttrValue(r"A<>\"'=;()Z"))
    assert p == {"type": "Property", "value": r"A<>\"'=;()Z"}


def test_prop_string_escaped():
    p = Attr()._build_property(AttrValue(r"A<>\"'=;()Z"), escape=True)
...
def test_geoprop_point_as_tuple():
    p = Attr()._build_geoproperty((44.84044, -0.5805))
    assert p == {
        "type": "GeoProperty",
        "value": {"coordinates": [-0.5805, 44.84044], "type": "Point"},
    }
...
def test_temporal_prop_datetime():
    p = Attr()._build_temporal_property(datetime(2021, 8, 31, 12, tzinfo=timezone.utc))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_datetime_str():
    p = Attr()._build_temporal_property("2021-08-31T12:00:00Z")
    assert p == {
        "type": "Property",
        "value": {"@type": "DateTime", "@value": "2021-08-31T12:00:00Z"},
    }
...
def test_temporal_prop_date():
    p = Attr()._build_temporal_property(date(2021, 8, 31))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_date_str():
    p = Attr()._build_temporal_property("2021-08-31")
    assert p == {
        "type": "Property",
        "value": {"@type": "Date", "@value": "2021-08-31"},
    }
...
def test_temporal_prop_time():
    p = Attr()._build_temporal_property(time(12, 0, 0))
    assert p == {
        "type": "Property",
...
def test_temporal_prop_time_str():
    p = Attr()._build_temporal_property("12:00:00Z")
    assert p == {
        "type": "Property",
        "value": {"@type": "Time", "@value": "12:00:00Z"},
    }
...
def test_temporal_prop_str_bad_format():
    with pytest.raises(ValueError):
        p = Attr()._build_temporal_property("25:00:00Z")
